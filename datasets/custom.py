...
from .datahelpers import pil_loader, imresize, cid2filename
...
def build_transform(input_size=224, mode='train'):
    t = []
...
class TuplesDataset(data.Dataset):
    ...
    def __init__(self, data_root, mode, imsize=None, nnum=5, qsize=2000, poolsize=20000, transform=None, loader=pil_loader):
        ...
        db_path = os.path.join(data_root,f'{mode}.pkl')
        ...
        self.images = [os.path.join(data_root,db['images'][i]) for i in range(len(db['images']))]

        
        # initializing tuples dataset
        self.mode = mode
        self.imsize = imsize
        self.cat_ids = db['category']
        self.qpool = db['query_indxs']
        self.ppool = db['base_indxs']
        ...
        self.transform = build_transform(imsize, self.mode)
        self.loader = loader
    ...
    def create_epoch_tuples(self, net):
        ...
        with torch.no_grad():
            ...
            qvecs = torch.zeros(384, len(self.qidxs)).cuda()
            for i, input in enumerate(loader):
                qvecs[:, i] = net(input.cuda())[0]
            ...
            for q in range(len(self.qidxs)):
                ...
                qcluster = self.cat_ids[self.qidxs[q]]
                cat_ids = [qcluster]
                nidxs = []
                r = 0
                while len(nidxs) < self.nnum:
                    ...
                    if not self.cat_ids[potential] in cat_ids:
                        nidxs.append(potential)
                        cat_ids.append(self.cat_ids[potential])
